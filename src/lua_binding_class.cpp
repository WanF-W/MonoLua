...
namespace
{
    ...
    int Class_GetFullName(lua_State* state)
    {
        ...
        const char* nameSpace = resolver.ClassNamespace(userdata->klass);
        const char* name = resolver.ClassName(userdata->klass);
    ...
    int Class_GetParent(lua_State* state)
    {
        ...
        LuaBridge_PushClass(
            state,
            MonoResolver::Instance().ClassParent(userdata->klass));
        return 1;
    }
    ...
    int Class_IsValueType(lua_State* state)
    {
        ...
        lua_pushboolean(
            state,
            MonoResolver::Instance().ClassIsValueType(userdata->klass));
        return 1;
    }
    ...
    int Class_IsEnum(lua_State* state)
    {
        ...
        lua_pushboolean(
            state,
            MonoResolver::Instance().ClassIsEnum(userdata->klass));
        return 1;
    }
    ...
    int Class_GetAddress(lua_State* state)
    {
        ...
        lua_pushinteger(
            state,
            static_cast<lua_Integer>(reinterpret_cast<uintptr_t>(userdata->klass)));
        return 1;
    }
    ...
    int Class_GetMethod(lua_State* state)
    {
        ...
        for (MonoMethod* method : resolver.EnumerateMethods(userdata->klass))
        {
    ...
    int Class_GetField(lua_State* state)
    {
        ...
        LuaBridge_PushField(
            state,
            MonoResolver::Instance().FindField(
                userdata->klass,
    ...
    MonoClassField* FindStaticField(MonoClass* klass, const char* name)
    {
        auto& resolver = MonoResolver::Instance();
    ...
    int Class_StaticCall(lua_State* state)
    {
        ...
        for (MonoClass* current = userdata->klass;
            current; current = resolver.ClassParent(current))
        {
            ...
            if (ambiguous)
                return luaL_error(state,
                    "ambiguous static overload: %s; use Class:get_method() and Method:call()",
                    name);
            if (matched) return LuaBridge_InvokeMethod(state, matched, nullptr, 3);
        }
        return luaL_error(state, "static method not found or arguments are incompatible: %s", name);
    }
    ...
    int Class_FindUnityObjects(lua_State* state)
    {
        ...
        if (!MonoScheduler::IsMainThread())
            return luaL_error(state,
                "find_unity_objects() must run inside mono.schedule() on the Unity main thread");
        std::vector<uint32_t> handles;
        std::string error;
        if (!MonoRuntime::Instance().FindUnityObjects(userdata->klass, handles, error))
            return luaL_error(state, "%s", error.c_str());

        lua_createtable(state, static_cast<int>(handles.size()), 0);
        for (size_t index = 0; index < handles.size(); ++index)
        {
            // handle 的所有权在这里转交给 Instance userdata，由其 __gc 释放。
            LuaBridge_PushInstanceHandle(state, handles[index]);
            lua_rawseti(state, -2, static_cast<lua_Integer>(index + 1));
        }
        return 1;
    }
    ...
    {
        const LuaClassUD* userdata = LuaBridge_CheckClass(state, 1);
        if (MonoResolver::Instance().ClassIsValueType(userdata->klass))
            return luaL_error(state, "alloc() does not support value types");
        MonoObject* object = MonoResolver::Instance().NewObject(
            MonoRuntime::Instance().Domain(), userdata->klass);
        if (!object) return luaL_error(state, "failed to allocate Mono object");
    ...
    int Class_New(lua_State* state)
    {
        const LuaClassUD* userdata = LuaBridge_CheckClass(state, 1);
        if (MonoResolver::Instance().ClassIsValueType(userdata->klass))
            return luaL_error(state, "new() does not support value types");
        auto& resolver = MonoResolver::Instance();
        ...
        {
            const char* name = resolver.MethodName(method);
            if (!name || strcmp(name, ".ctor") != 0) continue;
            const auto parameters = resolver.MethodParameters(method);
            const int score = LuaBridge_ScoreMethodArguments(state, 2, parameters);
            if (score > bestScore)
            {
                bestScore = score;
                constructor = method;
        ...
        if (ambiguous)
            return luaL_error(state, "constructor overload is ambiguous");
        if (!constructor) return luaL_error(state, "matching constructor was not found");
        MonoObject* object = resolver.NewObject(
            MonoRuntime::Instance().Domain(), userdata->klass);
        if (!object) return luaL_error(state, "failed to allocate Mono object");
        ...
        const int results = LuaBridge_InvokeMethod(state, constructor, object, 3);
    ...
    int Class_NewArray(lua_State* state)
    {
        ...
        if (length < 0) return luaL_error(state, "array length cannot be negative");
        MonoArray* array = MonoResolver::Instance().NewArray(
            MonoRuntime::Instance().Domain(), userdata->klass,
            static_cast<uintptr_t>(length));
        if (!array) return luaL_error(state, "failed to create Mono array");
    ...
    int Class_Dump(lua_State* state)
    {
        ...
        output << (resolver.ClassName(userdata->klass) ? resolver.ClassName(userdata->klass) : "?");
        const auto fields = resolver.EnumerateFields(userdata->klass);
        const auto methods = resolver.EnumerateMethods(userdata->klass);
        output << "\n\nFields (" << fields.size() << "):\n";
        for (MonoClassField* field : fields) output << "  " << resolver.FieldSignature(field) << '\n';
        output << "\nMethods (" << methods.size() << "):\n";
        for (MonoMethod* method : methods) output << "  " << resolver.MethodSignature(method) << '\n';
    ...
    int Class_ToString(lua_State* state)
    {
        ...
        sprintf_s(
            ...
            static_cast<unsigned long long>(
                reinterpret_cast<uintptr_t>(userdata->klass)));
