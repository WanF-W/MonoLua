 ...
 * 调用统一经过 lua_value 参数编组与 mono_runtime_invoke。
 * ============================================================
 */
...
namespace
{
    ...
    int Method_Call(lua_State* state)
    {
        ...
        if ((resolver.MethodFlags(userdata->method) & METHOD_ATTRIBUTE_STATIC) == 0)
        {
            object = LuaBridge_GetInstanceObject(state, 2);
            if (!resolver.ObjectIsInstanceOf(object, resolver.MethodClass(userdata->method)))
                return luaL_error(state, "instance is not compatible with the method's declaring class");
            firstArgument = 3;
        }
    ...
    int Method_GetAddress(lua_State* state)
    {
        ...
        if (!address) { lua_pushnil(state); return 1; }
    ...
    {
        const LuaMethodUD* userdata = LuaBridge_CheckMethod(state, 1);
        if (!MonoHook::UnhookMethod(userdata->method))
            return luaL_error(state, "method is not hooked");
        return 0;
    }
}
...
const luaL_Reg* LuaBinding_GetMethodMethods()
{
    static const luaL_Reg methods[] = {
        ...
        {"__tostring", Method_GetSignature},
        {nullptr, nullptr}
    };
    return methods;
}
